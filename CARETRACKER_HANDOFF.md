...
> **Current version:** v78 on the work branch, v77 live. v78 makes the password box on a
> password-protected backup typable — it was throwing the caret out after every keystroke. v77 made
> the phone's own Back button walk the app instead of closing it. Before those:
> a red overdose warning is no longer replaced by an amber timing notice, and
...
|---|---|---|---|
| v78 | Sep 15, 2026 | **The password box on a protected backup could not be typed in.** `onInput` on `#bk-unlock-pw` called `setState`, which ends in an unconditional `render()`, and this app's `render()` does not restore focus — it protects typing by suppressing the one-second tick while a field is focused, which does nothing about a repaint the handler asks for itself. One character per tap, on the screen that restores an encrypted backup. The file already stated the rule at line 3692 and every other input obeyed it; this was the only violation. **Found by fixing dead default paths, not by reading:** seven harness suites pointed at `harness/work/`, a v43.3-era build directory deleted months ago, so they died on ENOENT and looked retired — they are not, and two went red on v77 the moment they could run. `harness/encbackup-test.mjs` covers this screen thirteen other ways and never saw it, because its `type()` helper sets the whole value and fires one input event; **ENC-9b presses keys one at a time and asserts the caret survives — red on v77, green on v78.** No write path, envelope format or key derivation touched. Built by `harness/v78-bkpw-patch.py`, which reproduces this build from v77 byte-identically. |
| v77 | Sep 14, 2026 | **The phone's own Back button walks the app instead of closing it.** Aaron found it; searching this app, staging and ChemoWell for any handling of it returned nothing, so it had never been built — Rule 5.5's class exactly, since every gate here asks about a still frame and none asks what happens while a finger is moving. Back closes one layer at a time across 22 registered layers, then goes to Home, and leaves only with nothing open, on one press rather than two. **Writes no record of any kind.** Registry plus a suite that reads the app's own `state` to prove no layer is missing; six exemptions each with a reason, and one printed coverage gap (`medsync.confirm`, asserted by inspection). Built by `harness/v77-back-button-patch.py`. |
| v76 | Sep 14, 2026 | **A safety warning can no longer be pushed off the screen.** `state.warn` is a single slot and the iron/protonix branch set it and returned before any ceiling check ran, so the red acetaminophen banner could be replaced moments later by an amber timing note with nothing left to say it had been there. The more serious warning wins now, and "Take all" checks every medication in the batch rather than only Iron. **This row was missing from the handoff until v77 added it** — the release shipped without its own history entry. Built by `harness/v76-warning-priority-patch.py`. |
| v75 | Sep 11, 2026 | **Removed medications can be brought back.** Aaron picked this off the Enhancer's list on 2026-09-11: *"Do A, B and C."* **The gap was structural and had been open for many releases.** The app has carried `archivedMeds` since long before this — removing a medication keeps its name so old doses still render properly — but **no screen listed them and nothing brought one back.** A medication paused between cycles had to be typed in again, and came back under a NEW id, so every dose that referenced the old one reads as a removed medication. The data was never lost; it was unreachable. **THE WRITE MODEL, stated before a line was written:** this release appends nothing, and no code path in it writes, edits or deletes an ENTRY. It changes the medication CONFIG only — `meds` and `archivedMeds` — through the existing `persistMedicationConfig()`, the path every medication edit already uses. **The archive now keeps the WHOLE medication** (`config`) beside the `{name, sub}` it already kept; nothing that used to be kept is dropped, and an older build still finds exactly what it expects. There is precedent — ChemoWell widened this same record once before, to keep `pausePeriods`, and had to add the matching line to `normalizeArchivedMeds` because otherwise the next load stripped it straight back out. **Restore puts the medication back under its ORIGINAL id**, which is the entire point. **REMINDERS COME BACK EXACTLY AS THEY WERE, AND WHAT IS SUPPRESSED IS THE SPAN IT WAS AWAY — BOTH ENDS OF IT.** Removing a medication records the day it left (`removedAt`); bringing it back turns that into an `awayPeriods` span running from that day to this one, and `missedDosesFor()` skips a day only when the day falls INSIDE one of those spans. **The audit refused this release twice, and the second refusal was worse than the first.** The second design stamped `alertsFrom` and skipped everything BEFORE it — but the span a medication was archived for is only ever part of *everything before now*, so bringing one back erased every missed dose it had ever had. Measured on the test fixture, where the medication is off the list for about two seconds: **122 misses spanning two months vanished — from the banner, from the History day summaries, and from the report that goes to the doctor (122 rows → 0)** — one-way, invisible, permanent, under a button labelled *Bring back*, in a release whose own copy promised the opposite. The first end cannot be recovered at restore time, which is exactly why the broken version reached for *everything before now*; it has to be written down when the removal happens, and now it is. **An archive written by an older build carries no `removedAt`, and that restore now records NO SPAN AT ALL.** It gave a single-day span until the sixth audit pass, and a single day is not nothing: measured, it took both of a tracked medication's windows off the restore day and two rows off the clinician export, while the toast, the row and this row of the changelog all said the days would still show as missed. Three false statements, so the code moved rather than the words — if the app does not know when the medication left, it suppresses nothing. **Said out loud:** a window missed earlier today while the medication was off the list therefore counts as missed on that path. Over-reporting, visible, and exactly what the caregiver is told. **The first version of this release got that wrong and the independent audit refused it, twice over in opposite directions.** It brought the medication back with reminders switched OFF: in ChemoWell that was erased at the next app open, because its normaliser RECOMPUTES `alerts` from the schedule type and never reads what was saved — so the flood was live; in care-tracker it stayed off FOREVER, under a toast promising *"turn them back on in Edit"* when **the medication editor has no reminders control at all**. Bringing back Protonix, Buspirone, Paroxetine, Iron or Dexamethasone would have silently and permanently killed its missed-dose alerting, under a button labelled *Bring back* — the v66 copy failure again, this time over a real loss of safety cover. **The design was wrong, not only the code:** *reminders off* trades a visible, recoverable problem — a wall of missed doses for days she was not taking it — for an invisible, unrecoverable one, and a clinician takes the first every time. **Said out loud:** a phone still on the old build ignores `awayPeriods` and shows the days it was away as missed until it updates; that is visible and self-correcting, and silent loss of alerting is neither. **Also said out loud, and the audit was right that this matters more than it looks:** an archive entry written by ANY EARLIER BUILD carries no `removedAt`, so it restores with **no span at all** and suppresses nothing — and since no build before this one could list removed medications, the first medication anybody brings back is necessarily one of those. The app knows which case each row is in and **says so on the row and in the toast** rather than promising something it cannot do; the first version of this release promised it unconditionally in three places, which is the v66 failure exactly. The span is also built from the **device clock**, so a phone with the wrong date records a wrong span. **And a span cannot be cleared — not by any route, including the one two drafts of this row claimed.** Restore appends, nothing removes, no screen shows a span, and bringing a medication back a second time adds a second one rather than replacing the first. **Deleting the medication does not undo it either:** removing one ARCHIVES it, spans and all, and bringing it back re-adds them; `delete archivedMeds[id]` appears exactly once in the file, inside restore. There is no purge control anywhere in this app, by design — a control that edits suppression is a control that can hide real missed doses. **This was written down wrongly twice**, first as *"correctable by removing and restoring again"* and then as *"stays until the medication is deleted outright"*, and the audit refused both; the second had got as far as the shipped source comment, where it was the stated reason it is safe to reject a bad span rather than clamp it. **What actually makes that safe, measured rather than asserted:** no entry is written, edited or deleted by any of this, every logged dose stays exactly where it is, and the suppression is **derived at render time** — drop the span and every count returns exactly. **What a span DOES change is every surface fed by `missedDosesFor()`**: the banner, the medication card's MISSED label, Today's journal, the History rows and day summaries, and the derived missed-dose rows in the clinician export. **The sentence that stood here before said a span changes "only what the banner counts" and that "the export reads the entries themselves". It was false** — on a 14-day span the export drops 305 rows to 277 and 14 History days change their MISSED count — and it contradicted this release's own guard comment, which lists the day summaries and the report to the doctor by name. **That was the fifth safety claim in this one release the audit refused for describing a property the code does not have**, and the fourth written specifically to replace the last one. The rule that came out of it is in STATUS.md. **Stripping the spans at removal would make the sentence true and is deliberately NOT done:** a medication removed for three months, restored, then removed and restored again would lose the first suppression, and three correctly-quiet months would flood back as missed. **The day of the restore is itself inside the span** (the range is inclusive at both ends), so a dose window already missed earlier that same day is not counted: one day, deliberate, and stated here because an exemption nobody wrote down is indistinguishable from an oversight. **THE ROW SAYS WHEN IT LEFT, and that is the same change as the audit's first blocker.** The Enhancer's one finding from pass 06 was that the removed row showed a name and a generic name and nothing else, although this release starts recording the removal date. The audit's blocker was that three strings promised, unconditionally, that only the days a medication was off the list go uncounted — untrue for every archive entry written before this release, which is **every entry that existed on upgrade day**, since no earlier build could list removed medications at all. Measured with the archive stripped exactly as a pre-v75 build leaves it: 305 → 183 → **305**, export 122 → **122**. One change answers both: the row that can say *"Removed 14 days ago · those days will not count as missed"* is the row that can say *"Removed before this update — the app cannot tell when, so those days will still count as missed"* instead. The toast and the changelog line are conditional now too, and the blurb over the list makes no promise at all. **SPANS ARE VALIDATED ON EVERY LOAD** — medsync accepts a medication config published by another phone, and the missed-dose walk should not be the thing that trusts it. The first version of that filter rejected only MALFORMED spans and the new check caught it within the minute: `{start: 1, end: <far future>}` passed every condition and swallowed the whole record, 305 → 183. A span this app writes always ends on the day of the restore, so an end beyond now is rejected. **Said out loud:** a span ending in the PAST cannot be checked this way — it is indistinguishable from a medication genuinely off the list for a long time — and the narrow way one could arise here is a phone whose date was wrong at the moment of REMOVAL and was corrected afterwards. What a span in that state would cost is bounded by the paragraph above — the surfaces fed by `missedDosesFor()`, under-reporting rather than over-, with no entry written, edited or deleted and every logged dose left where it is. **This sentence carried the false "only the banner" claim too, in the same two files that had just been corrected for it** — found by doing what the new rule says and grepping the file for what it already says about the same thing, which is exactly the step whose absence let the claim ship in six places to begin with. **Tie-break:** an active medication already holding that id refuses the restore by name — restoring over it, or under a new id, would silently sever the dose history. Restoring twice is a no-op. **An archive written by an older build** (name and generic name only) restores from the medication the app ships with, and the row SAYS *"doses and rules were not kept"* rather than pretending. **New gate `harness/archived-meds-test.mjs` 51/51**, and every assertion that matters reads the SAVED config rather than the screen — a screen can render the right word while the record holds the wrong one, which is the v43.3 failure class. **Falsifying it found two real gaps in the suite itself, both fixed:** nothing asserted the archived settings SURVIVE A RELOAD, so a build whose loader stripped them on the way back in scored a full green board; and the first version of that new check read `localStorage` and **passed on the broken build**, because the strip happens in memory on load and is only written back on the next save — the file still held what the app had already forgotten. It reads the row's own note now, which is what the app actually believes. The suite also edits the medication BEFORE removing it, so *"the archive kept what was removed"* cannot pass on a build that quietly archived the shipped default instead of the caregiver's own version; and it picks its test medication out of the file under test, because the app does not persist its config until something changes and a fresh device has nothing in storage to pick from. **AND THE SUITE'S OWN SAFETY CHECK WAS WORTHLESS, which took three attempts to put right.** It counted `[data-missed-row], [data-missed-banner]` — **neither selector exists in this app** — so it scored 0 against 0 on every build, broken or not: a check that could not fail, in the one file whose whole subject is checks that cannot fail. The second version counted the medication's name inside the banner, and the banner collapses to three days, so it counted VISIBLE rows and moved for reasons unrelated to the thing under test. The third returned *unreadable* both when the banner was absent — zero missed doses, which is an ANSWER — and when a build's banner carries no count, so three checks quietly exempted themselves the moment the number reached zero. It reads the count in the banner's own heading now, and the FIRST reading, taken while the fixture is guaranteed to have missed doses on screen, decides whether the build can be read at all. **305 before → 183 with the medication removed → 305 after bringing it back** — back to exactly where it started, because this medication was away for no days at all. Asserting that last number equalled the REMOVED one was green on the build that erased two months of history; comparing it to the BEFORE number is what catches that. **AND THAT CHECK COULD ONLY FAIL IN ONE DIRECTION, which breaking it on purpose is the only thing that would ever have shown.** It proves the suppression is not too WIDE and says nothing about whether it happens at all: delete the guard from the missed-dose walk outright and all 37 checks stayed green. So the suite now also backdates the archive to read as though the medication left a fortnight ago — what a real phone's would say — and brackets the result from both sides: **305 → 277** after bringing it back, below the untouched number because those fourteen days are not missed doses, and above the 183 it reads with the medication removed because every day outside the span is still counted. 28 suppressed is two Protonix windows across the days in that span that had both windows behind them — the span is fifteen days counting both ends, and the restore day contributed nothing at the hour the sweep ran, so the figure moves with the clock. The checks assert inequalities for that reason; the arithmetic is an illustration, not a constant. One mutant now goes red on each side. `outputs/SUITES-v75.md`: 0 failing. `overflow-scan` CLEAN. `APP_VERSION` → `v75`, CACHE → `caretracker-v75`. Rollback bundle for v74 in `outputs/rollback-v74/`. |
