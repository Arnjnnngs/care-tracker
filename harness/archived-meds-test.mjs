...
{
  // Section 3 proves the suppression is not too WIDE. This proves it happens at all.
  // The medication there is off the list for about two seconds, so deleting the guard from the
  // missed-dose walk outright changed no number and every check stayed green -- found by breaking
  // it on purpose, which is the only way that kind of hole is ever found.
  // Here the archive is made to say what a real phone's would say after two weeks: it left on a day
  // in the past. Bringing it back must drop the total (those days are not missed doses) and must
  // NOT drop it to the removed number (every day outside the span is still counted).
  await goMeds();
  await clickLabel('Remove ' + TRACKED.name);
  await page.waitForTimeout(400);
  await clickLabel('Confirm removal of ' + TRACKED.name);
