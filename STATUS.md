...
|---|---|
| **Version** | v73 |
| **Commit** | `705121a` — v73 LIVE on main. Removing a corrected reading no longer restores the wrong number. |
| **URL** | https://arnjnnngs.github.io/care-tracker/ |
| **index.html md5** | `05c5af8afcdd393862daf362d525a10f` |
| **sw.js md5** | `08af1a1947bc65292c293ee179a589d6` |
| **State** | **v73 — removing a corrected reading no longer brings the wrong number back.** Home's journal and History rendered raw documents, so on a corrected weight or paracentesis the row standing there was the CORRECTION and its Remove hard-deleted that one document. **Inside 48 hours the rules allow that delete**, so it succeeded and the group fell back to what it superseded — remove a reading, and the mistyped one returns on the screen a clinician reads a trend off. Both v72 auditors named it; neither could block on it because it predates v72 (v67/v69). Fixed by routing weight and paracentesis to the tombstone functions the report screens already use, with a fall-through so the button can never become a silent no-op; `removeBtn` also hides on a superseded weight. **A second, older defect the new suite found while proving the first:** `removeParacentesis` stamped its tombstone with a bare `Date.now()` where weight uses `Math.max(Date.now(), previous + 1)` — with two phones' clocks apart the tombstone lost the tie and the procedure stayed on screen while the toast said *Paracentesis removed*. Same guard now. `remove-group-test` **20/20, and 10/20 against v72**; it seeds the correction TODAY on purpose, because the 48-hour window where a delete WORKS is the window where it does damage. **The audit BLOCKED the first build on the CHECK, not the code** — the new suite's stamp guard could not fail after noon, because its seed came off the frozen app clock. Fixed to clear both clocks and proved hour-independent: the reverted build is red at +0h, +6h, +13h and +21h. `outputs/SUITES-v73.md`: 30 PASS, 4 EXEMPT, 0 failing. The delta audit SHIPPED it after one further copy correction: *"older readings"* was true of a weight and false of a paracentesis. `overflow-scan` 140/140 CLEAN. **Exempt:** iPhone rendering — Chromium only here. **Needs Aaron's phone:** correct a weight, then remove it, and confirm the old number does not return. |
