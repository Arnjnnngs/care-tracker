...
| | |
|---|---|
| **Version** | **v78 is LIVE on `main`.** Aaron gave the go-ahead on 2026-09-16 (*"push v78"*) and it went out the same turn. v77 before it, on 2026-09-14. |
| **Commit** | v78 = `07632e4` on `main`. The deployed `sw.js` on `main` reads `caretracker-v78`, confirmed through the GitHub API. **This sandbox cannot reach `arnjnnngs.github.io` at all** — the proxy refuses the CONNECT tunnel with a 403 — so the Pages run plus the file on `main` IS the confirmation, and a session that reports fetching the live page here is reporting something it did not do. (Previously: v77 = `b5a27db`. GitHub Pages build and deployment concluded **success** (run 167). The deployed `sw.js` on `main` reads `caretracker-v77`, confirmed through the GitHub API — **this sandbox cannot reach `arnjnnngs.github.io` at all** (the proxy refuses the CONNECT tunnel with a 403), so the Pages run plus the file on `main` IS the confirmation, and a session that reports fetching the live page here is reporting something it did not do. |
| **URL** | https://arnjnnngs.github.io/care-tracker/ |
| **index.html md5** | `80ba7549395c78e216f2d4f919239630` (the v78 build in this tree) |
| **sw.js md5** | `feec1522f9b31a8ecd0129f732036763` (the v78 build in this tree) |
| **v78 — SHIPPED 2026-09-16** | **The password box on a password-protected backup could not be typed in.** `onInput` on `#bk-unlock-pw` called `setState`, `setState` ends in an unconditional `render()`, and this app's `render()` does not restore focus — it protects typing by suppressing the one-second tick while a field is focused, which does nothing about a repaint the handler asks for itself. One character per tap, on the screen that restores an encrypted backup of a medication history. **Found by fixing dead default paths, not by reading:** seven harness suites pointed at `harness/work/`, a v43.3-era build directory deleted months ago, so they died on ENOENT and looked retired. They are not — `cal-test` 70/70, `export-test` 49/49, `medskip-test` 10/10 against live v77 — and two went red on this one line the moment they could run. `harness/encbackup-test.mjs` covers this screen thirteen other ways and never saw it, because its `type()` helper sets the whole value and fires one input event; **ENC-9b presses keys one at a time, RED on v77 and GREEN on v78.** Reproducible byte-identically from v77 by `harness/v78-bkpw-patch.py`. **Pushed to `main` on Aaron's word, 2026-09-16.** What remains is his own phone: the backup-restore password box has never been typed into on the device this app is installed on. |
| **State (v77, LIVE)** | **The phone's own Back button closed the app from anywhere.** Aaron found it: *"All apps close out (go to user phone home screen) when hitting the phones built in back button... Should have been found already."* He is right — Rule 5.5 says every gate here asks about a still frame, and nobody asks what happens while a finger is moving. `grep popstate` returned nothing in any of the three apps: never built. Back now closes one layer at a time (22 registered), then goes to Home, and only leaves with nothing open — on one press, not two. **Writes no record of any kind.** `harness/back-button-test.mjs` 14/14 with one printed coverage gap (`medsync.confirm`, 1 of 22, asserted by inspection). Render scan 140/140 clean. **Two suites still read FAIL and both were measured as PRE-EXISTING on the shipped v76 build**, not caused here. |
| **State (v76)** | **A red overdose warning was being silently replaced by an amber timing notice, and it is live on `main` today.** The day reaches the 2,500 mg acetaminophen limit, one more Tylenol goes in through the app's own override, and the red *"Acetaminophen ceiling exceeded — do not give more without contacting the care team"* banner appears. The caregiver then taps **Take all** on the evening meds; Iron is in that batch, Protonix was logged within the last two hours, and the amber *"Iron + Protonix timing"* notice takes the red banner off the screen with nothing to say it was ever there. `state.warn` is one slot and the iron branch set it and returned before any ceiling check could run — **which warning she ended up looking at depended on what she happened to tap next.** `afterLog` now collects every warning a dose earns and shows the worst; an amber never displaces a red still on screen, a red may still replace a red. **Second defect:** *"Take all"* asked about Iron and nothing else, so a batch crossing any other medication's configured daily limit warned about nothing. **Write model: appends nothing, edits nothing, deletes nothing** — it changes which of two banners is on the screen and nothing else. `harness/warning-priority-test.mjs` **18/18**, falsified against v75 at **15/18**. **Needs Aaron's phone after it ships:** log Tylenol past the limit through the override, then tap Take all on the evening meds, and confirm the red banner is still the one on screen. |
