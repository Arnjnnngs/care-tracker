...
| | |
|---|---|
| **Version** | **v77 is LIVE on `main`.** Aaron gave the go-ahead on 2026-09-14 (*"#29, push to main"*) and it went out the same turn. |
| **Commit** | v77 = `b5a27db` on `main`. GitHub Pages build and deployment concluded **success** (run 167). The deployed `sw.js` on `main` reads `caretracker-v77`, confirmed through the GitHub API — **this sandbox cannot reach `arnjnnngs.github.io` at all** (the proxy refuses the CONNECT tunnel with a 403), so the Pages run plus the file on `main` IS the confirmation, and a session that reports fetching the live page here is reporting something it did not do. |
