...
| **State** | **v64 — the screen stops rebuilding itself every second.** Aaron: *"caretracker screen flickers to often on my Samsung. I assume bc it's trying to stay live with my phone and Brandi's iphone."* It was not the sync — measured with Firestore stubbed and silent, **ten full rebuilds in ten seconds**, ~20ms of main-thread stall each and ~30ms with six months of history, so it worsens as the record grows. Nothing on the resting screen shows seconds, so ~59 of every 60 rebuilds changed nothing. The tick still runs (checkNotifications rides on it); it now repaints only when a signature of what is visible changes — the minute, which medications are locked, and the second while the override countdown is on screen. **Ten rebuilds per ten idle seconds → zero.** `repaint-test` **17/17** asserts the other half: the clock still moves, a medication still unlocks the second it is due, and the *“Opens in …”* countdown still ticks `35s -> 32s`. The audit returned **SHIP** and named three things to fold in, all of them now in the file: the tick is wrapped in `try/finally` so a throw inside `render()` cannot leave the gate stuck on and silently swallow the next tap; the seconds countdown got the test it never had; and `tour-test` got its own `TICK-positive-control` (60/70, same ten pre-existing `--file` failures as on shipped v63). The auditor then re-verified the delta — **SHIP**, every number reproduced independently — and found two more, both in the test files: a check that could not tell a live sabotage hook from a dead one, and a comment of mine claiming three working checks could no longer fail. Both fixed; the app file is unchanged since that re-verify. Rollback bundle for v63 in `outputs/rollback-v63/`. |
